function [L,C,U,LUT,H,Imin,Imax]=FastFCMeans(im,c,q)
disp('FastCMeans')
...
Imin=double(min(im(:)));
Imax=double(max(im(:)));
im_min_max_vector=(Imin:Imax)';

% Compute intensity histogram
H=hist(double(im(:)),im_min_max_vector);
H=H(:);
...
im_min_max_vector=repmat(im_min_max_vector,[1 c]); dC=Inf;
while dC>1E-6
    
    C0=C;
    
    % Distance to the centroids
    D=abs(bsxfun(@minus,im_min_max_vector,C));
    D=D.^(2/(q-1))+eps;
    ...
    C=sum(UH.*im_min_max_vector,1)./sum(UH,1);
